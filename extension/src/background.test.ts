...
describe('background tab isolation', () => {
  ...
  it('maps legacy workspace commands to session and surface', async () => {
    const { chrome } = createChromeMock();
    vi.stubGlobal('chrome', chrome);

    const mod = await import('./background');

    expect(mod.__test__.normalizeIncomingCommand({
      id: 'legacy-browser',
      action: 'tabs',
      op: 'list',
    ...
    expect(mod.__test__.normalizeIncomingCommand({
      id: 'legacy-site',
      action: 'exec',
      workspace: 'site:buyandship',
      code: '1',
    })).toEqual(expect.objectContaining({
      session: 'site:buyandship',
      surface: 'adapter',
    ...
    expect(mod.__test__.normalizeIncomingCommand({
      ...
      action: 'bind',
      session: 'bound:example',
      surface: 'browser',
    }));
  });

  it('accepts legacy workspace commands without throwing session_required', async () => {
    const { chrome } = createChromeMock();
    chrome.tabs.query = vi.fn(async () => []);
    vi.stubGlobal('chrome', chrome);

    const mod = await import('./background');
    mod.__test__.setAutomationWindowId(
      mod.__test__.getLeaseKey('site:buyandship', 'adapter'),
      1,
    );
