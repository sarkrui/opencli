...
/** Map legacy workspace-based daemon commands to session + surface. */
function normalizeIncomingCommand(cmd: Command): Command {
  let normalized: Command = cmd.action === 'bind-current'
    ? { ...cmd, action: 'bind' }
    : cmd;

  if (normalized.session?.trim()) return normalized;

  const workspace = normalized.workspace?.trim();
  if (!workspace) return normalized;

  if (workspace.startsWith('browser:')) {
    return {
      ...normalized,
      session: workspace.slice('browser:'.length) || 'default',
      surface: normalized.surface ?? 'browser',
    };
  }
  if (workspace.startsWith('operate:')) {
    return {
      ...normalized,
      session: workspace.slice('operate:'.length) || 'default',
      surface: normalized.surface ?? 'browser',
    };
  }
  if (workspace.startsWith('site:')) {
    return {
      ...normalized,
      session: workspace,
      surface: normalized.surface ?? 'adapter',
      siteSession: normalized.siteSession ?? 'persistent',
    };
  }
...
async function handleSessions(cmd: Command): Promise<Result> {
  const now = Date.now();
  const data = await Promise.all([...automationSessions.entries()].map(async ([, session]) => {
    let tabCount = 0;
    if (session.windowId !== null) {
      const tabs = await chrome.tabs.query({ windowId: session.windowId });
      tabCount = tabs.filter((tab) => isDebuggableUrl(tab.url)).length;
...
async function handleCommand(rawCmd: Command): Promise<Result> {
  const cmd = normalizeIncomingCommand(rawCmd);
